...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<input class="danmu-input ctrl-btn"   type="textarea" id="danmu_text" max=300 />'); // -> button A
        ...
        this.$ctrlMain.append('<div class="volume  ctrl-btn-right"><span class="glyphicon glyphicon-volume-up" aria-hidden="true"></span></div>');
        ...
        $("body").append('<div id="' + this.id.slice(1, this.id.length) + 'fontTip"  hidden="true">' +
            ...
            '<input type="radio" checked="checked"  name="danmu_position" value=0 />Scrolling&nbsp;&nbsp;<input type="radio" name="danmu_position" value=1 />Top' +
            '&nbsp;&nbsp;<input type="radio" name="danmu_position" value=2 />Bottom&nbsp;&nbsp;</form>' +
            '<form  id="danmu-size" >Font Size：<input   type="radio" checked="checked"  name="danmu_size" value="1" />Large&nbsp;&nbsp;' +
        ...
        this.volumeMuted = false; //true = muted (active)
        this.isLoop = false;
        this.danmuSize = 0;
        ...
        $(this.id + " .show-danmu").scojs_tooltip({
            appendTo: this.id,
            content: 'On/off comments'
        });
        $(this.id + " .volume").scojs_tooltip({
            appendTo: this.id,
            content: 'Volume'
        });
        ...
        //Get Danmu from backend (<-query.php <-MySQL database)
        this.getDanmu = function () {
            $.get(that.options.urlToGetDanmu, function (data, status) {
                //$.get in jQuery will collect all 'echo' info from query.php, 
                //so unneccessary echo must be commented out. 
                //What we get should be a clean json array
                //e.g.['{ "text":"test","color":"#ffffff","size":"1","position":"0","time":3}','{ "text":"test","color":"#ffffff","size":"1","position":"0","time":58}']
                console.log(data);
        ...
        if (options.urlToGetDanmu){
            console.log(options.urlToGetDanmu);
            this.getDanmu();
            //$.get( "query.php");
        }   
        ...
        this.sendDanmu = function (e) {
            ...
            if (e.data.that.options.urlToPostDanmu){
                ...
                console.log(textObj);
                //Post Danmu to backend (->stone.php -> MySQL database)
        ...
        //mute button event
        $(this.id + " .volume").on("click", {that: that}, function (e) {
            if (!e.data.that.volumeMuted) {
                console.log(e.data.that.volumeMuted);
                e.data.that.volumeMuted = true;
                console.log(e.data.that.volumeMuted);
                e.data.that.video.volume = 0;
                $(e.data.that.id + " .volume").addClass("ctrl-btn-right-active");
            }
            else {
                console.log(e.data.that.volumeMuted);
                e.data.that.volumeMuted = false;
                console.log(e.data.that.volumeMuted);
                e.data.that.video.volume = 1;
                $(e.data.that.id + " .volume").removeClass("ctrl-btn-right-active");
            }
